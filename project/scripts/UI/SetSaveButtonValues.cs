...
public class SetSaveButtonValues : MonoBehaviour
{
    ...
    public void UpdateSaveSlots()
    {
        ...
        API_manager.Instance.GetPlayers();
    ...
    {
        SaveManager.Instance.LoadFile(indexToRetrieve);
        var saveCode = SaveManager.Instance.saveCode;
        if (saveCode != "")
        {
            main_text.text = $"Сохранение игры {indexToRetrieve}";
            small_text.text = $"Сид мира и значения ресурсов:{saveCode.Split(";")[0]}";
        }
        else
        {
            main_text.text = $"Пустая ячейка сохранения";
            small_text.text = "Вы можете создать новую игру здесь";
        }
    }
